...
def cached(f):
    ...
    def inner(*args, **kwargs):
        for memory in queue:
            if(memory[0] == args):#Is cached
                return memory[1]
        
        if(len(queue) != 3):#Isnt cached and queue isnt full
            queue.append((args,f(*args, **kwargs)))
...
def parser_string(stringToFind: str) -> Parser[str]:
    """
    ...
    Example:
        ...
        ```
    """
    def parse(targetString: str):
        resultString = ""
        parser = parser_matches(lambda x: x in (stringToFind))
        resultOfMatch = parser(targetString)
        while(True):
            if(resultOfMatch.value is None):
                break
            resultString = resultString + resultOfMatch.value
            if(resultString == stringToFind):
                break
            resultOfMatch = parser(resultOfMatch.rest)
        if(resultString == stringToFind):
            return ParseResult(resultString, resultOfMatch.rest)
        else:
            return ParseResult(None, targetString)
        
    return parse
parser = parser_string("abc")
print(parser("abca"))# => ParseResult(value="foo", rest="x")
print(parser("fo"))# => ParseResult(value=None, rest="fo")
def parser_int() -> Parser[int]:
    ...
    Example:
        ...
        ```
    """
    def parse(stringToSearch: str):
        resultString = ""
        parser = parser_matches(lambda x: x in ("0123456789"))
        resultOfMatch = parser(stringToSearch)
        while(True):
            if(resultOfMatch.value is None):
                break
            resultString = resultString + resultOfMatch.value
        ...
        if(len(resultString) == 0):
            return ParseResult(None, stringToSearch)
        else:
            return ParseResult(int(resultString), resultOfMatch.rest)
    return parse
parser = parser_int()
print(parser("123x"))# => ParseResult(value=123, rest="x")
print(parser("foo"))# => ParseResult(value=None, rest="foo")
